...
from network.msvit import Attention


def count_attention_cell(m: Attention, x: torch.Tensor, y: torch.Tensor):
    B, H, G, N, d = x[0].shape
    total_ops = B* H * G  * N * d * N
    total_ops += B * H * G * N * N * 4
    total_ops += B * H * G * N * N * d * m.sparse_k
...
parser.add_argument('--sparse-ratio', type=float, default=1.0)
parser.add_argument('--sparse-pool', type=str2bool, default=True)
parser.add_argument('--split-size', type=str, default="8,8")
            ...
            if opt.amp:
                with torch.cuda.amp.autocast():
                    ...
                    for idx, output in enumerate(outputs):
                        weight += 1
                        losses.append(1 * criterion1(output, label_list))
...
if __name__ == '__main__':
    ...
    net = DBN(num_classes=num_classes, num_parts=[1,opt.num_part], std=opt.std, net=opt.net, erasing=opt.erasing, h=opt.img_height//16, w=opt.img_width//16, sparse_ratio=opt.sparse_ratio, split_size=opt.split_size, sparse_pool=opt.sparse_pool, stride=opt.stride, drop_path=opt.drop_path)
    ...
    for _ in range(5):
        os.system("python test_{}.py --net {} --img-height {} --img-width {} --batch-size 32 --gpus {} --dataset-root {} --num-part {} --sparse-ratio {} --split-size {} --sparse-pool {} --stride {}".format(opt.dataset, opt.net, opt.img_height, opt.img_width, opt.gpus, opt.dataset_root, opt.num_part, opt.sparse_ratio, opt.split_size, opt.sparse_pool, opt.stride))
