...
def extract_feature(net, dataloaders):
    ...
    for img, _ in dataloaders:
        ...
        count += n
        print(count)
        ff = np.zeros((n, 512*(opt.num_part+3)), dtype=np.float32)
        for i in range(2):
            if(i==1):
...
if __name__ == '__main__':
    ...
    net = DBN(num_classes=665, num_parts=[1,opt.num_part], net=opt.net, h=opt.img_height//16, w=opt.img_width//16, sparse_ratio=opt.sparse_ratio, split_size=opt.split_size, sparse_pool=opt.sparse_pool, stride=opt.stride)
