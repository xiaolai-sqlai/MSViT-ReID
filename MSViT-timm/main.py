...
def get_args_parser():
    ...
    parser.add_argument('--model', default='tiny', type=str, metavar='MODEL',
                        help='femto, pico, nano, tiny, small, base')
...
def main(args):
    ...
    if args.model == "tiny":
        model = MSViT(dims=[32,64,128,256], layers=[3,6,15,3], ratio=4.0, num_head=2, split_size=7, sparse_ratio=1.0, act_type="gelu", drop_path_rate=0.05)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema = evaluate(data_loader_val, model_ema.module, device, use_amp=args.use_amp)
    ...
    '''if args.model_ema and args.model_ema_eval:
        utils.bn_update(data_loader_train, model_ema.module, use_amp=args.use_amp)
        test_stats_ema = evaluate(data_loader_val, model_ema.module, device, use_amp=args.use_amp)
        print(f"Accuracy of the model EMA on {len(dataset_val)} test images: {test_stats_ema['acc1']:.1f}%")
        if max_accuracy_ema < test_stats_ema["acc1"]:
            max_accuracy_ema = test_stats_ema["acc1"]
